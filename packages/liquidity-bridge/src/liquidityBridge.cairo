...
pub mod LiquidityBridge {
    ...
    impl LiquidityBridgeImpl of ILiquidityBridge<ContractState> {
        ...
        fn swap_fiat_to_token(
            ...
            assert(token_amount > 0, bridge_errors::INVALID_AMOUNT);
            assert(fiat_amount > 0, bridge_errors::INVALID_AMOUNT);

            let token = self.token_by_symbol.read(token_symbol);
            assert(!token.is_zero(), bridge_errors::INVALID_TOKEN);
            let mut token_info = self._get_token_info(token);
            ...
            IERC20Dispatcher { contract_address: token }.transfer(user, token_amount);

            // decrease recorded liquidity by constructing a new TokenInfo with updated total
            if token_info.total_liquidity >= token_amount {
                let symbol = token_info.symbol;
                let decimals = token_info.decimals;
                let min_amount = token_info.min_amount;
                let max_amount = token_info.max_amount;
                let is_active = token_info.is_active;
                let feed_id = token_info.feed_id;
                let added_at = token_info.added_at;
        ...
        fn swap_token_to_fiat(
            ...
            assert(price_per_token > 0_u256, bridge_errors::CANNOT_BE_ZERO);
            ...
            token_info.total_liquidity += token_amount_after_fee;
            self.tokens.write(token, token_info);
