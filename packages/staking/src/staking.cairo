...
pub mod SyncStaking {
    ...
    use crate::errors::staking_errors as StakingErrors;
    use crate::events::{
        BalanceMerkleRootUpdated, EmergencyWithdrawal, FiatRewardClaimRecorded, FiatStakeRecorded,
        FiatUnstakeRecorded, PoolCreated, PoolUpdated, ReserveSnapshotCreated, RewardsClaimed,
        RewardsDeposited, Staked, Unstaked,
    };
    use crate::interfaces::{FiatStake, ISyncStaking, StakePosition, StakingPool};
    ...
    struct Storage {
        ...
        // Fiat staking
        user_fiat_stake_count: Map<
            (ContractAddress, felt252), u64,
        >, // (user, token_symbol) => count        
        fiat_stakes: Map<
            (ContractAddress, felt252, u64), FiatStake,
        >, // (user, currency, stake_id) => FiatStake
    ...
    impl SyncStakingImpl of ISyncStaking<ContractState> {
        ...
        fn unstake(
            ref self: ContractState, user: ContractAddress, token_symbol: felt252, stake_id: u64,
        ) {
            self.pausable.assert_not_paused();
            self.reentrancy.start();
            ...
            assert(
                treasury_balance >= stake.accumulated_rewards, StakingErrors::INSUFFICIENT_REWARDS,
            );
        ...
        fn record_fiat_unstake(
            ref self: ContractState, user: ContractAddress, currency: felt252, stake_id: u64,
        ) {
            self.ownable.assert_only_owner();
        ...
        fn create_reserve_snapshot(
            ref self: ContractState, currency: felt252, balance: u256, ipfs_hash: felt252,
        ) {
            self.ownable.assert_only_owner();
