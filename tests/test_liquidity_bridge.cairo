...
fn setup() -> (ContractAddress, ILiquidityBridgeDispatcher, SyncTokenDispatcher) {
    ...
    let (token_address, token) = deploy_erc20("etherium", "ETH");

    start_cheat_caller_address(token_address, owner());
    token.mint(owner(), 500 * 10_u256.pow(18)); // Mint tokens to owner
    token.approve(bridge_address, 500 * 10_u256.pow(18));
    stop_cheat_caller_address(token_address);

    let (token_address, token) = deploy_erc20("starknet", "STRK");
    ...
    bridge.register_token('ETH'.into(), token_address);
    bridge.add_fiat_liquidity('USD'.into(), 1_000_000 * 10_u256.pow(18));
    bridge.add_token_liquidity('ETH'.into(), 500 * 10_u256.pow(18));
